...
class AiotManager:
    ...
    async def async_unload_entry(self, config_entry: ConfigEntry):
        """Unload a config entry and detach its entities."""
        devices_in_entry = self._entries_devices.get(config_entry.entry_id, [])
        platforms: list[str] = []
        for did in devices_in_entry:
            device = self._managed_devices.get(did)
            if not device or not device.is_supported:
                continue
            for platform_map in device.platforms:
                platforms.extend(platform_map.keys())
        ...
        if unload_ok:
            ...
            self._entries_devices.pop(config_entry.entry_id, None)
