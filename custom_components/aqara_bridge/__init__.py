...
async def async_setup_entry(hass, entry):
    ...
    if (
        ...
        if isinstance(resp, dict) and resp.get("code") == 0:
            ...
            raise ConfigEntryAuthFailed("Unable to refresh Aqara token")
    else:
        aiotcloud.set_country(data.get(CONF_ENTRY_AUTH_COUNTRY_CODE))
    ...
    try:
        ...
        raise ConfigEntryNotReady(f"Failed to initialize Aqara devices: {ex}") from ex
...
async def async_unload_entry(hass, entry):
    manager: AiotManager = hass.data[DOMAIN][HASS_DATA_AIOT_MANAGER]
    return await manager.async_unload_entry(entry)
